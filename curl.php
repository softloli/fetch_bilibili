...
class CURL{
	...
	*/
	function add($url=array(),$p=array(),$f=array()){
		...
		if(!is_array($url) or empty($url[0])){
			debug_print('url is invalid, '.print_r($url,true),E_USER_ERROR);
		}else{
			//url中的空格中加号替换，这是一个容易出问题的地方
	...
	*/
	...
	/**
	* add a task to curl,全自动保持并发数在这里实现
	*/
	private function addTask(){
		...
		while($c>0){
			...
			if(!empty($this->taskFailed)){
				...
				*/
				if(0<(int)($this->limit-count($this->taskPool)) and isset($this->task)){
						call_user_func($this->task);
				}
